	...
	public static double[][] main(int n, int K,double[] L,String shearormoment,String endsupports) {
		...
		for (int e = 0; e< n; e++) {
			...
			if(shearormoment.length()==6 && endsupports.length()==18) {
				...
				*/
				...
				//Global Matrix
				float [][] global= new float[2*n+3][2*n+3];
					...
					for (int i = 0; i < n; i++) {
						...
						if (i<=K-1) {
						 ...
						 l=2*i+1;
							...
							for (int m = 0; m < 4; m++) {
									for (int r = 0; r < 4; r++) {
										if (k==2*K-1&&i==K-1) {k=k+1;}
										if (l==2*K-1&&i==K-1) {l=l+1;}
				...
				float[] force= new float[2*n+3];
				force[2*K-1]=1;
				force[2*K]=-1;
				
				//Defining Displacement Matrix
				float[] displacement= new float[2*n+3];
		...
		//Finding Final Equation
				...
				System.out.println();
				float Scalefactor = displacement[2*K]-displacement[2*K-1];
				System.out.println("Values of Scalefactor: " + Scalefactor);  System.out.println(); 
				
				valueofy = new double[101*n] ;
				double X1;
				int r= 0;
			...
			if(shearormoment.length()==6 && endsupports.length()==22) {
				...
				*/
				...
				// GlobalK1 Matrix
				float globalk1[][] = new float[n+1][n+1];
				int l=0;
				int q=0;
				for (int i = 0; i <n+1; i++) {
				...
				// Inverse of GlobalK1 Matrix 
				float inverse[][] = new float[n+1][n+1];
				inverse = Inverse.invert(globalk1);
				System.out.println();
				System.out.println("Inverse of globalK1 Matrix");
				...
				for (int i = 0; i < n+1; i++) {
						...
						for (int j = 0; j < n+1; j++) {
							if (i<=K-2) {
								if (j<=K-2){displacement[2*i+3]+=inverse[i][j]*force[2*j+3];}
								if (j>=K-1)  {displacement[2*i+3]+=inverse[i][j]*force[2*j+2];}
							
							}
					
							if (i>=K-1) {
								if (j<=K-2){displacement[2*i+2]+=inverse[i][j]*force[2*j+3];}
		...
		//Finding Final Equation
			...
			if(shearormoment.length()==5 && endsupports.length()==18) {	
				...
				*/
				...
				//Global Matrix
				float [][] global= new float[2*n+2][2*n+2];
				...
				// Defining Force Matrix
				float[] force= new float[2*n+2];
				force[2*K-2]=1;
				
				//Defining Displacement Matrix
				float[] displacement= new float[2*n+2];
				...
				//Storing vertical force
						System.out.println("Verical Forces :");
						float verticalforce[]=new float[n+1];
						int j=0;
						for (int i = 0; i < n+1; i++) {
							if (i<=K) {verticalforce[i]= force[2*j];}
							j++;
		...
		//Finding Final Equation
				...
				System.out.println();
				float Scalefactor = displacement[2*K-2];
				System.out.println("Values of Scalefactor: " + Scalefactor);  System.out.println(); 
				...
				*/
				...
				if (K==1) {
					...
					for (int i = 0; i < n+1; i++) {
							...
							for (int j = 0; j < n+1; j++) {
								if (i==0) {
									if (j==0) 	{displacement[0]+=inverse[i][j]*force[0];}
									if (j>0) 	{displacement[0]+=inverse[i][j]*force[2*j+1];}
								
								}
							
								if (i>0) {
									if (j==0)  {displacement[2*i+1]+=inverse[i][j]*force[0];}
									if (j>0) 	{displacement[2*i+1]+=inverse[i][j]*force[2*j+1];}					
								}
			...
			//Finding Final Equation
				...
				for (int i = 0; i < n+1; i++) {
						...
						for (int j = 0; j < n+1; j++) {
							if (i<K-2) {
								if (j<K-2) 	{displacement[2*i+3]+=inverse[i][j]*force[2*j+3];}
								if (j==K-2)	{displacement[2*i+3]+=inverse[i][j]*force[2*j+2];}
								if (j>=K-1) {displacement[2*i+3]+=inverse[i][j]*force[2*j+1];}
							...
							if (i>=K-1) {
								if (j<K-2)    {displacement[2*i+1]+=inverse[i][j]*force[2*j+3];}
